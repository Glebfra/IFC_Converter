...
namespace IFCConverter.Importer.IfcToDomain.EntityPortResolvers.AvevaEntityPortResolvers
{
    internal sealed class AvevaElbowEntityPortResolver : IAvevaEntityPortResolver
    {
        ...
        public void Resolve(IIfcProduct product, EngineeringModel model, ImportContext context)
        {
            ...
            Vector<double> axisGlobalPosition = areaMatrix.GetRotation().LeftMultiply(axisLocalPosition) + areaMatrix.GetOffset();
            axisGlobalPosition *= lengthPower;

            Vector<double> axis = (elbow.Position - axisGlobalPosition).Normalize(2);
            Vector<double> upDirection = axis.CrossProduct(areaMatrix.GetY()).Normalize(2);
            
            Matrix<double>[] rotationMatrices =
            {
                MatrixExtensions.CreateRotationAroundVector(upDirection, revolvedAreaSolid.Angle / 2).GetRotation(),
                MatrixExtensions.CreateRotationAroundVector(upDirection, -revolvedAreaSolid.Angle / 2).GetRotation()
            };

            Vector<double>[] portPositions = rotationMatrices
                .Select(matrix => matrix.Multiply(axis * elbow.Radius) + axisGlobalPosition)
                .ToArray();
            Vector<double>[] portDirections = portPositions.Select(position => (position - elbow.Position).Normalize(2)).ToArray();
